...
def test_provider_switching():
    ...
    if os.getenv("OPENAI_API_KEY"):
        providers_to_test.append(("dashscope", os.getenv("OPENAI_API_KEY")))
        print("✅ Alibaba Cloud Model Studio API key found")
    else:
        print("⏭️  Skipping Alibaba Cloud Model Studio (no API key)")

    if os.getenv("OPENAI_API_KEY"):
        providers_to_test.append(("siliconflow", os.getenv("OPENAI_API_KEY")))
        print("✅ SiliconFlow API key found")
    else:
        print("⏭️  Skipping SiliconFlow (no API key)")

    if os.getenv("OPENAI_API_KEY"):
        providers_to_test.append(("doubao", os.getenv("OPENAI_API_KEY")))
        print("✅ Doubao API key found")
    else:
        print("⏭️  Skipping Doubao (no API key)")

    if os.getenv("OPENAI_API_KEY"):
        providers_to_test.append(("kimi", os.getenv("OPENAI_API_KEY")))
        print("✅ Kimi API key found")
    else:
        print("⏭️  Skipping Kimi (no API key)")

    if os.getenv("OPENAI_API_KEY"):
        providers_to_test.append(("deepseek", os.getenv("OPENAI_API_KEY")))
