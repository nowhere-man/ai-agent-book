...
class Config:
    ...
    # Kimi API Settings (also accept OPENAI_API_KEY for compatibility)
    OPENAI_API_KEY: str = os.getenv("OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY", "")
    ...
    def get_evaluator_config(cls, evaluator: Optional[str] = None) -> dict:
        ...
        if evaluator == "kimi":
            ...
            raise ValueError(
                "OPENAI_API_KEY not configured. Set OPENAI_API_KEY/OPENAI_API_KEY "
                "or OPENAI_API_KEY (fallback) in .env file."
            )
            ...
            prefer_openrouter = (bool(cls.OPENAI_API_KEY)
            ...
            raise ValueError(
                ...
                "OPENAI_API_KEY (fallback) in .env file."
            )
        else:
