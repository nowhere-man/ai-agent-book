...
- Preferred credentials: `OPENAI_API_KEY`, fallback to `OPENAI_API_KEY` (`gpt-*` remapped to `openai/*`).
...
export OPENAI_API_KEY=your-openai-api-key   # or OPENAI_API_KEY=your-openrouter-api-key
python demo.py
...
- 默认模型 **`gpt-5.6-luna`**（当前廉价旗舰），通过 openai Python SDK 调用。首选 `OPENAI_API_KEY`；未设置时**自动回退到 `OPENAI_API_KEY`**（走 OpenRouter 兼容端点，`gpt-*` 映射为 `openai/*`）。由于 `gpt-5.x` 直连 OpenAI 需组织实名认证，只要存在 `OPENAI_API_KEY` 就优先走 OpenRouter。
...
export OPENAI_API_KEY=your-openai-api-key           # 或 export OPENAI_API_KEY=your-openrouter-api-key（自动回退）
python demo.py
...
- 凭据：首选 `OPENAI_API_KEY`；未设置时自动回退到 `OPENAI_API_KEY`（走 OpenRouter，`gpt-*` 映射为 `openai/*`）。`gpt-5.x` 直连需组织实名认证，故有 `OPENAI_API_KEY` 时优先走 OpenRouter。离线复算（`--offline`）无需任何 key。
