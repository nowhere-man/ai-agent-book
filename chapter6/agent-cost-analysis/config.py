...
1. 默认模型为 gpt-5.6-luna（当前廉价旗舰）。首选凭据是 OPENAI_API_KEY；若未设置，
   自动回退到 OPENAI_API_KEY 并把模型名映射成 OpenRouter id（gpt-* -> openai/*）。
   由于 gpt-5.x 直连 OpenAI 需要组织实名认证，只要 OPENAI_API_KEY 存在就优先走
...
# OpenRouter 回退：无 OPENAI_API_KEY 时用 OPENAI_API_KEY 走 OpenAI 兼容端点。
...
def make_client_and_model(model: str):
    ...
    回退策略（universal OpenRouter fallback）：
      - gpt-5.x 且存在 OPENAI_API_KEY -> 优先走 OpenRouter（直连需组织实名认证）；
      - 否则有 OPENAI_API_KEY -> 直连 OpenAI，模型名不变；
      - 否则有 OPENAI_API_KEY -> 走 OpenRouter，模型名按 _to_openrouter_model 映射；
      - 两者皆无 -> 抛出清晰错误。
    """
    ...
    if orkey:
        return (
            OpenAI(base_url=OPENAI_BASE_URL, api_key=orkey,
    ...
    raise RuntimeError(
        ...
        "OPENAI_API_KEY（自动回退到 OpenRouter）；或改用 --offline 离线复算（无需 key）。"
    )
