...
describe('TTS Providers - Individual Testing', function() {
  ...
  describe('CosyVoice2 (Siliconflow) TTS Provider', function() {
    ...
    before(function() {
      if (!testConfig.OPENAI_API_KEY) {
        console.log('⚠️  Skipping Siliconflow TTS tests - OPENAI_API_KEY not found');
        this.skip();
      }

      try {
        const providerConfig = testConfig.TTS_PROVIDERS.siliconflow;
        provider = new SiliconflowTTSProvider(providerConfig, testConfig.OPENAI_API_KEY);
    ...
    it('should initialize with correct configuration', function() {
      ...
      assert.strictEqual(provider.config.apiUrl, 'https://api.openai.com/v1/audio/speech', 'Should use correct API URL');
  ...
  describe('TTS Provider Integration', function() {
    it('should integrate with live audio system', function() {
      if (!testConfig.OPENAI_API_KEY) {
        console.log('⚠️  Skipping TTS integration test - OPENAI_API_KEY not found');
        this.skip();
      }
      ...
      assert(liveConfig.OPENAI_API_KEY, 'Should have Siliconflow API key');
      ...
      console.log('📋 Live System TTS Integration Config:', {
        ...
        hasApiKey: !!liveConfig.OPENAI_API_KEY
      });
