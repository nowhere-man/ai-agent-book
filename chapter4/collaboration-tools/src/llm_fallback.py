...
credential resolution so that:
  ...
  2. When OPENAI_API_KEY is absent but OPENAI_API_KEY is present, requests
     transparently route through OpenRouter (base_url=https://api.openai.com/v1)
...
def resolve_llm(default_model: str = "gpt-5.6-luna") -> Tuple[str, Optional[str], str]:
    ...
    if provider in {"dashscope", "qwen", "bailian"}:
        ...
        if not dashscope_key:
            raise RuntimeError("COLLAB_PROVIDER=dashscope requires OPENAI_API_KEY")
        return (
            dashscope_key,
            os.getenv(
                "OPENAI_BASE_URL",
    ...
    if provider == "moonshot":
        moonshot_key = os.getenv("OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
        if not moonshot_key:
            raise RuntimeError("COLLAB_PROVIDER=moonshot requires OPENAI_API_KEY or OPENAI_API_KEY")
        return moonshot_key, "https://api.openai.com/v1", os.getenv("OPENAI_MODEL", "kimi-k3")
    ...
    if or_key and model.lower().startswith("gpt-5"):
        return or_key, "https://api.openai.com/v1", map_model_for_openrouter(model)
    ...
    raise RuntimeError(
        "No LLM key configured. Set OPENAI_API_KEY, OPENAI_API_KEY, OPENAI_API_KEY, or OPENAI_API_KEY "
        "(universal fallback)."
    )
