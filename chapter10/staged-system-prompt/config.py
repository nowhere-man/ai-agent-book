...
class Config:
    ...
    def validate(cls) -> None:
        ...
        0) gpt-5.x 系列直连 OpenAI 需组织验证（且这类推理模型只接受默认温度），
           因此只要设置了 OPENAI_API_KEY，即便同时有 OPENAI_API_KEY，也优先走
           OpenRouter，省去组织验证的麻烦；
        1) 否则有 OPENAI_API_KEY -> 直连 OpenAI（尊重 OPENAI_BASE_URL）；
        2) 否则有 OPENAI_API_KEY -> 改走 OpenRouter，并映射模型名；
        3) 都没有则报清晰错误。
        """
        ...
        if cls.API_KEY:
            return
        raise SystemExit(
            "错误：未检测到 OPENAI_API_KEY 或 OPENAI_API_KEY 环境变量。\n"
            "请先 `export OPENAI_API_KEY=...`（或 OPENAI_API_KEY），"
