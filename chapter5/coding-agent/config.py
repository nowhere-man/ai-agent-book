...
class Config:
    ...
    def map_model_to_openrouter(model: str) -> str:
        ...
        Used by the OpenRouter fallback so a user with ONLY an OPENAI_API_KEY
    ...
    def resolve(cls) -> dict:
        ...
        OPENAI_API_KEY is available, transparently route through OpenRouter
        ...
        if requested in ("anthropic", "openai"):
            ...
            raise ValueError(
                f"No {key_name} set and no OPENAI_API_KEY available for fallback. "
                f"Set {key_name} for direct access, or set OPENAI_API_KEY to route "
